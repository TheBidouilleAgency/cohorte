...
async function main() {
  ...
  if (!verb || verb === 'help') {
    console.log(`cohorte doctor [--codex /path/to/codex]
cohorte prepare --repo /repo --profile /profile.json --out /new-preparation-directory
cohorte check --repo /repo --config /preparation/config.json
cohorte run --repo /repo --config /config.json --spec /spec.md
cohorte status|logs|diff <run-id> --repo /repo
cohorte resume <run-id> --repo /repo --acknowledge-uncertain
