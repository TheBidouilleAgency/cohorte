...
for (const f of workflowNames) {
  // `ci &&`: same guard as every other ci.yml cross-check in this file. Under legacy/v2
  // there is no ci.yml beside the sources (the V3 workflow no longer dry-runs the V2
  // installers), and this was the one check that treated "absent" as "asserts nothing".
  if (ci && !ci.includes(`workflows/${f}`))
