...
| `@cohorte/project-model` (`packages/project-model`) | V3.0 subset (D10): deterministic scan, `init` plan/apply, desired state, five-class field model, six-class drift diff for `reconcile --plan`, provenance/hash guard. Semantic discovery = seam. | `scanRepository`, `planInit`, `applyInit`, `planReconcile`; `./contract`: `ProjectModel`, `DriftReport`, `ReconcilePlan` `[S]` | base, config, git · yaml, typebox |
...
export interface CommandPayloads {
  ...
  reconcile:{ mode: 'plan' | 'apply' };                                // V3.0: 'plan' only
...
| reconciliation | `project-model/test/reconcile.itest.ts` | `--plan` is read-only (tree digest unchanged), human edit ⇒ CONFLICT entry |
