...
5. The importer (`init --export-v2`, `init --from-v2`) is V3.1; the seam is `project-model/src/import/` with a reserved bundle schema. The
   complete procedure, bundle contract and rollback rules are frozen in `docs/v3/MIGRATION.md`.
...
- V3.1 is released → decide when `legacy/v2/` leaves the repository; never remove it before the importer and its rollback tests ship.
