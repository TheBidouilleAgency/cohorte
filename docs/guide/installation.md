...
Cohorte requiert Python 3.12 ou plus récent. Le paquet **Python sur PyPI** s’appelle `cohorte-engine` ; la commande installée s’appelle `cohorte`. Ce guide vise la préversion `1.0.0a11`. **N’utilisez pas `npm install -g cohorte` : ce nom correspond à l’ancien paquet npm, pas à ce moteur Python.**

Avec `uv` :

```bash
uv tool install 'cohorte-engine==1.0.0a11'
cohorte --version
cohorte doctor
```
...
```bash
uv tool install 'cohorte-engine[claude]==1.0.0a11'
```
