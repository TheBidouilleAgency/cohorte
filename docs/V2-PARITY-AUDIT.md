...
| Spec | Existing agent draft plus per-question acceptance of proposed answers and project overview; `spec-propose` exposes the same read-only proposal and durable references to structured clients | A live Codex read-only proposal on a synthetic repository produced cited scenarios and valid surface/check IDs; terminal interview, structured draft editing/approval, guided editing of existing scenarios/criteria and Claude remain unqualified |
