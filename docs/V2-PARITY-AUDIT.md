...
| Brainstorm | Project-configured panel, repository overview and cited search every round, focused question proposals with business/code options, acceptance shortcuts | Two two-round live Codex scenarios passed (see both `evidence/g5-codex-v2-parity-*.json` files); Claude and more varied real-project decisions remain unqualified |
| Spec | Existing agent draft plus per-question acceptance of proposed answers and project overview; `spec-propose` exposes the read-only proposal and `spec-draft` converts an explicitly accepted, fresh proposal into an editable draft while retaining open questions | A live Codex proposal, `spec-propose`, `spec-draft` and freeze-request on a synthetic repository produced valid references, scenarios and surface/check IDs; deterministic tests cover stale-brief rejection. Terminal interview, final approval/freeze, guided editing of existing scenarios/criteria and Claude remain unqualified |
| Build, review, fix and fleet | Fresh project overview alongside task-specific repository evidence; supervised Fleet plan/status/sync now provisions isolated worktrees, checks remote branch drift, and rebases clean idle branches after a proven merge | Real provider runs over multi-surface projects, external session liveness and integration order; Fleet status does not yet show phase-level run evidence |
