...
| Build and delivery | `start` verifies a guided frozen spec and current profile, then asks before launching a live worktree run; `incoming-review NUMBER` reviews an existing PR/MR in a detached checkout without publishing a forge comment | Ship remains a separate explicit approval and command; Fleet and expert runs still need explicit paths and IDs. |
| Patch and maintenance | `patch-spec --from-intake` proposes a bounded read-only diagnosis; `audit` can use the current project profile and directories. `retro` mines recurring review findings, offers an advisory rule and waits for ratification. `patch`, `refactor` and `align-ds-*` remain explicit. | Refactor and design flows still need guided scope and evidence review; write paths and approvals remain explicit. |
