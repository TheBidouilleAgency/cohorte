# Cohorte Next — premier noyau natif
...
Reconstruction indépendante de la V3 : **spec fournie → build → tests → review → fix borné → résultat prêt à examiner**.
TypeScript, XState, SQLite et Codex app-server. Aucun import des packages V3.
...
Le run affiche immédiatement sa branche et son worktree. L'état est dans `~/.cohorte-next/<identifiant-projet>/state.sqlite`, hors du worktree. `--state-root /autre/emplacement` permet de changer la racine ; conserver la même option pour consulter le run.
